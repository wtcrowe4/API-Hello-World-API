...
//Seed
languages.get('/seed', (req, res) => Language.insertMany(
    [
        {
            "name": "english",
    ...
    .then(languages => res.json({msg: "Languages added successfully"}))
...
//languages.get('/random', (req, res) => {
    ...
    // console.log(random)
    //.skip(random)
    // Language.find().random(1)
    //     .then(language => res.json(language))
    //     .catch(err => res.status(400).json('Error: ' + err))
//})

//Index 
languages.get('/', (req, res) => Language.find()
    .then(languages => res.json(languages))
    .catch(err => res.status(400).json('Error: ' + err))
)

//Show
languages.get('/:name', (req, res) => Language.findOne({ name: req.params.name .toLowerCase()})
    .then(language => res.json(language))
    .catch(err => res.status(400).json('Error: ' + err))
)
